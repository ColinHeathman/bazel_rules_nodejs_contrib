...
func (lang *JS) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	aggregateModule := !lang.Config.NoAggregateModules && module
	if aggregateModule {
	...
	if len(webAssets) > 0 {
		...
		// record all webAssets rules for  web_app_assets rule later
		fqName := fmt.Sprintf("//%s:%s", path.Join(args.Rel), name)
		lang.WebResources[fqName] = true
	}
	if isWebRoot {
		// Generate web_app_assets rule
		webRootDeps := []string{}
		for fl := range lang.WebResources {
			webRootDeps = append(webRootDeps, fl)
		}
		name := "web_app_assets"
		r := rule.NewRule("copy_to_bin", name)
